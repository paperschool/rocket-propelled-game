...
const isScript = (assetFileName: string): boolean =>
    assetFileName.substring(assetFileName.length - 3, assetFileName.length) === '.js';
...
export const scriptPaths = (): ScriptProperty[] =>
    Object.entries(manifestProvider())
        .reduce(
            (scriptAssets, [assetKey, { src, integrity }]) => [
                ...scriptAssets,
                isScript(assetKey) ? { src, integrity } : undefined,
            ],
            []
        )
        .filter(Boolean);
